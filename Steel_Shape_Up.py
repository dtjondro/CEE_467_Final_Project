...
T1 = tk.Label(root,text="Which design pholosophy do you follow?",bg = "#C1CDCD")
...
R_Compression_Members.grid(row=7, column=0, sticky="W")
...
def new_winF():
    ...
    def Bending_Design():
        ...
        type_of_load_l.get() == '' 
            ...
            if z_value != 0:
                ...
                bending_shape1.grid(row=3, column=0, sticky="W",pady = (0,40))
            ...
            if z_value != 0:
                ...
                bending_shape_final2.grid(row = 9, column=0, sticky="W",pady = (0,40))
            
            quote["text"] = dict[random.randint(1,6)]
            quote.grid(row = 10, column=0, sticky="W")
    ...
    def Compression_Design():
        ...
        Entry_4.get() != '' and (Entry_1.get() != '' or Entry_2.get() != '' or Entry_3.get() != '')):
            ...
            compression_final_shape2.grid(row = 9, column = 0, sticky = "W",pady = (0,40))
            
            quote["text"] = dict[random.randint(1,6)]
            quote.grid(row = 11, column=0, sticky="W")
    ...
    def Connections_Design():
        ...
        type_of_bolt.get() == '' or type_of_connection.get() == '' or
        Bolt_Diameter.get() != '' and (Parallel_length.get() != '' or Transverse_length.get() != '')):
            ...
            if type_of_connection.get() == "Bolt":
                ...
                area.grid(row=1, column=0, sticky="W",pady = (0,5))
                strength["text"] = "For shear, the design strength = " + str("%.3f" % design_strength) + " kips."
                strength.grid(row=2, column=0, sticky="W",pady = (0,45))
                ...
                p_strength.grid(row=1, column=0, sticky="W",pady = 5)
                t_strength["text"] = "The transverse weld strength = " + str("%.2f" % t_str) + " kips."
                t_strength.grid(row=2, column=0, sticky="W",pady = 5)
                
                weld_str = max(p_str + t_str, 0.85 * p_str + 1.5 * t_str)
                strength["text"] = "The weld strength = " + str("%.2f" % weld_str) + " kips."
                strength.grid(row=3, column=0, sticky="W",pady = (0,40))
    ...
    def Tension_Design():   
        ...
        Entry_1.get() != '' and Entry_2.get() != '' and Entry_4.get() != ''):
            ...
            if method.get() == "LRFD":
                ...
                tension_l4.grid(row = 16, column = 0, sticky = "W", pady = (0,40))
            ...
            if method.get() == "ASD":
                ...
                tension_l3["text"] = "The combination of holes and shear lag may not reduce the area of this "
                tension_l4["text"] = "member by more than " + str("%.3f" % ((total_load * 2 /Fu)/(total_load * 1.67 /Fy)))
            ...
            quote["text"] = dict[random.randint(1,6)]
            quote.grid(row = 20, column=0, sticky="W")
...
button.grid(row=14, column=0, sticky="W", pady=(0,10))
